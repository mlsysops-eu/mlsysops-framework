...
def main():
    ...
    for cluster_name in inventory['all']['children']:
        try:
            ...
            logger.warning(f"Skipping cluster '{cluster_name}': {e}")
    ...
    for cluster_name in inventory['all']['children']:
        ...
        try:
            ...
            logger.warning(f"Skipping application YAML generation: {e}")

    # Generate continuum-level YAML
    try:
        create_continuum_yaml(input_file)
    except ValueError as e:
        logger.warning(f"Skipping continuum YAML generation: {e}")
