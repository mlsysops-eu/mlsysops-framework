...
@framework.command(help="fita-agents")
@click.option('--descriptions', type=click.Path(exists=True), required=False,
              help='Fita Node descriptions.')
def fita_agents(descriptions):
    try:
        deploy_fita_agents(descriptions)
    except Exception as e:
        click.secho(f"❌ Error during fita node agents deployment: {e}", fg='red')
...
framework.add_command(fita_agents)
