...
def create_cr(plan):
    ...
    #     logger.error(f" This is a the error 2 {traceback.format_exc()}")

    return app_name, wrapped


async def apply(plan):
    logger.debug(f"Applying Cluster Placement plan {plan}")

    app_id, cr_spec = create_cr(plan)
    ...
    async with kubernetes_asyncio.client.ApiClient() as api_client:
        ...
        # except Exception as e:
        #     logger.error(f" This is a the error {traceback.format_exc()}")
