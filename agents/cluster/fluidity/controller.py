...
def translate_plan(new_deployment_plan, old_specs, new_specs=None):
    ...
    if not app_modified:
        ...
        logger.info(f'new_deployment_plan {new_deployment_plan}')
        new_specs = []
    ...
    for component in new_specs:
        ...
        logger.info(f"old template {old_spec['pod_template']}")
        logger.info(f"new template {new_spec['pod_template']}")
    ...
    logger.info(f"Final deployment plan {new_deployment_plan}")
    return True
...
class FluidityAppController():
        ...
        while True:
            try:
                ...
                # From spade agent
                    ...
                    # TODO: Add this as Pod label
                    plan_uid = payload.get("plan_uid")

                    logger.info("plan_uid %s", plan_uid)
                ...
                match req['operation']:
                    ...
                    case MessageEvents.PLAN_SUBMITTED.value:
                        ...
                        if initial_plan:
                            ...
                            logger.info('Received new plan request %s', plan)
                            logger.info('curr plan %s', self.apps_dict[name]['curr_plan']['curr_deployment'])
    ...
    async def _handle_upd_app(self, app_name, new_app_spec, plan_uid):
        ...
        if new_app_spec != self.apps_dict[app_name]['spec']:
            ...
            logger.info(f'change spec from description: new_deployment_plan {new_deployment_plan}')
        else:
            ...
            if not res:
                ...
                self.apps_dict[app_name] = app_copy
                return False, None
            logger.info(f'change spec from policy: new_deployment_plan {new_deployment_plan}')
        ...
        for comp_name in new_deployment_plan['curr_deployment']:
            ...
            for entry in new_deployment_plan['curr_deployment'][comp_name]:
                ...
                if action == 'move':
                    ...
                    logger.info(f'Updated spec: {updated_spec}')
                    ...
                    logger.info(plan_dict[comp_name])                                                   
                else:
        ...
        logger.info(f'final dict is {plan_dict}')
