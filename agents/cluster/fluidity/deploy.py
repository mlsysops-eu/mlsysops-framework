...
from objects_util import dict2yaml
from config import send_notification_to_host, generate_all_policy_configs, generate_policy_configs, get_node_internal_ip, get_adhoc_info
from scheduler import available_cloud_nodes, SCHEDULER_NAME
from nodes import get_k8s_nodes, get_node_availability
...
def extend_pod_label_template(manifest, app_name, app_uid, comp_name, placement):
    ...
    `mlsysops.eu/appUID`, `mlsysops.eu/component` and `mlsysops.eu/componentUID`
    (completed later), and at spec field the `schedulerName` (not used) and
    `nodeAffinity` specifying the acceptable `mlsysops.eu/node-type` based on
    ...
    labels['mlsysops.eu/appUID'] = app_uid
...
def extend_pod_instance(manifest, pod_name, system_services, host_name=None):
    ...
    if system_services:
        ...
        manifest['spec']['hostNetwork'] = True
        app_uid = manifest['metadata']['labels']['mlsysops.eu/appUID']
        for container in manifest['spec']['containers']:
...
def set_pod_placement(manifest, node_type):
    ...
    manifest['spec']['nodeSelector']['mlsysops.eu/node-type'] = node_type
...
def update_pod_image(api, pod_name, new_img):
    ...
    try:
        api.patch_namespaced_pod(name=pod_name, namespace='default', body=patch)
    except ApiException as exc:
        logger.error('Failed to patch Pod: %s', exc)
        return

def reconfigure_deployment(app, plan):
    """Update Pod spec and policy configs for all selected component instances.

    Args:
        app (dict): The FluidityApp info dictionary.
        plan (list): Contains the dicts that correspond to the new
                              desired Pod specs.
    """
    ...
    for comp_name in plan:
        ...
        #logger.info('components %s', app['components'])
        logger.info(app['pod_names'])
        found = False
        ...
        comp_spec = app['components'][comp_name]
        for action_item in plan[comp_name]:
            if action_item['action'] == 'change_img':
                podname_to_patch = None
                for pod_name in app['pod_names']:
                    if pod_name.startswith(comp_name):
                        podname_to_patch = pod_name
                        break
                new_img = action_item['new_img']
                host = action_item['host']
                logger.info('Received change img to %s for component %s on host %s', new_img, comp_name, host)
                update_pod_image(api, podname_to_patch, new_img)
                comp_spec['pod_template']['spec']['containers'][0]['image'] = new_img
                logger.info('Updated pod template %s', comp_spec['pod_template'])
    ...
    for priority_entry in priority_list:
        for comp_name in initial_deployment:
            ...
            if comp_name in app['hybrid_mobile_comp_names']:
                ...
                system_services = bool('systemServices' in comp_spec)
                pod_template = comp_spec['pod_template']
                agent_msg[comp_name] = copy.deepcopy(comp_dict)
                agent_msg[comp_name]['qos_metrics'] = comp_spec['qos_metrics']
                for host in initial_deployment[comp_name]:
                    ...
                    # Update status for the selected host
                    host['status'] = 'ACTIVE'
                    agent_msg[comp_name]['specs'].append(pod_dict)
                ...
                host_name = initial_deployment[comp_name][0]['host']
