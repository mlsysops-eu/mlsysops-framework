...
from fluidity_nodes import set_fluiditynode_label, set_node_label
from fluidityapp_settings import DRONE_CANDIDATE_DISTANCE, EDGE_CANDIDATE_DISTANCE
from fluidityapp_settings import INTERSECTION_THRESHOLD, MOBILE_EDGE_PROXIMITY_RANGE, cluster_id
from fluidityapp_util import sort_linked_list
...
def find_candidates_drone(app, drones_list, dronestations_list, nodes_list):
    ...
    candidate.<app-name>.fluidity.gr/<component-name>.
    ...
    if app['has_driver']:
        ...
        for comp_name in app['drone_comp_names']:
            ...
            for drone_name in app['drone_candidates']:
                label = 'candidate.{}.fluidity.gr/{}'.format(app_name, comp_name)
...
def find_candidates_edge(app, comp_name, edgenodes_list, nodes_list):
    ...
    candidate.<app-name>.fluidity.gr/<component-name>-loc<location-num>.
            ...
            if is_point:
                ...
                if distance > EDGE_CANDIDATE_DISTANCE:
                    if edgenode['metadata']['name'] in comp_spec['candidates']:
                        ...
                        #label = 'candidate.{}.fluidity.gr/{}-loc{}'.format(app_name, comp_name, loc_num)
                    continue
            else:
...
def find_candidates_hybrid(app, comp_name, edgenodes_list, reachable_edgenodes, nodes_list, mobilenodes_list):
    ...
    Inserts labels to the respective drone and node k8s objects in the form
    candidate.<app-name>.fluidity.gr/<component-name> and
    hybrid.drone.<app-name>.fluidity.gr/<component-name> or
    hybrid.edge.<app-name>.fluidity.gr/<component-name>
    ...
    if comp_spec['is_drone_interacting'] and comp_spec['cluster_id'] == cluster_id:
        ...
        for edgenode in edgenodes_list:
            ...
            label1 = 'candidate.{}.fluidity.gr/{}'.format(app_name, comp_name)
            label2 = 'hybrid.drone.{}.fluidity.gr/{}'.format(app_name, comp_name)
            # NOTE: Setting node labels only when k8s node exists
            node_exists = False
            for node in nodes_list:
                if node.metadata.name == edgenode['metadata']['name']:
        ...
        for edgenode in edgenodes_list:
            ...
            if distance < MOBILE_EDGE_PROXIMITY_RANGE:
                ...
                if edgenode['metadata']['name'] not in comp_spec['candidates']:
                    ...
                    # Add labels
                    #label1 = 'candidate.{}.fluidity.gr/{}'.format(app_name, comp_name)
                    #label2 = 'hybrid.mobile.{}.fluidity.gr/{}'.format(app_name, comp_name)
                    # NOTE: Setting node labels only when k8s node exists
                    #node_exists = False
                    #for node in nodes_list:
                    #    if node.metadata.name == edgenode['metadata']['name']:
    ...
    for related_comp in comp_spec['hybridHeuristic']['relatedTo']:
        ...
        for edgenode_name in related_candidates_list:
            ...
            for edgenode2 in edgenodes_list:
                ...
                label1 = 'candidate.{}.fluidity.gr/{}'.format(app_name, comp_name)
                label2 = 'hybrid.edge.{}.fluidity.gr/{}'.format(app_name, comp_name)
