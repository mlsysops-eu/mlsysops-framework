...
from fluidityapp_util import sort_linked_list, human_to_byte
from fluidityapp_config import get_adhoc_info, get_node_internal_ip, send_notification_to_host
...
def available_cloud_nodes(nodes_list):
    ...
    for n in nodes_list:
        ...
        if n.metadata.labels['fluidity.gr/node-type'] != 'cloud':
            continue
    ...
    for pod in stream:
        if (pod['object'].status.phase == 'Pending'
            ...
            try:
                ...
                if pod['object'].spec.node_selector['fluidity.gr/node-type'] in ['drone', 'edge']:
