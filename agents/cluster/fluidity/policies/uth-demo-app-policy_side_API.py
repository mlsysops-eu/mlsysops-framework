...
model_url = "http://karmada.mlsysops.eu:10000/"
deployment_url = "http://karmada.mlsysops.eu:10000/"
# Get the hostname
...
def heuristic_func(model_input):
    ...
    current_action = ''
    try:
        average_latency = model_input["average_latency"]
        classifier_host = model_input["classifier_host"]
        vader_power_value = model_input["vader_power_value"]
        jetson_bottom_latency_threshold = model_input["jetson_bottom_latency_threshold"]
        vader_top_latency_threshold = model_input["vader_top_latency_threshold"]
        ...
        logger.error('Caugh exception %s on heuristic func', e)
    finally:
        return current_action
...
def analyze_status(app_desc, nodes, context, system_metrics, updated_nodes, curr_deployment):
    ...
    if context['model_info'] is not None:
        ...
        try:
            response = requests.post(context['model_info']['endpoint'], json=model_input)
            # Print response
