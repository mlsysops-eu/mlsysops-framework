        ...
        while True:
            try:
                ...
                if event and event == MessageEvents.MESSAGE_TO_FLUIDITY.value:
                    ...
                    logger.debug(f"Origin node: {origin_node} fluidity_internal_event: {fluidity_internal_event}  fluidity_internal_payload: {fluidity_internal_payload} ")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.debug("Messsage Validation successful!")
...
async def apply(plan):
    ...
    try:
        # Validate the payload using Pydantic
        logger.debug(f"Validating {plan}")
