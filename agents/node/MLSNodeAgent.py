    ...
    async def fluidity_proxy_message_listener(self):
        ...
        while True:
            try:
                ...
                logger.debug('====================Received from fluidity proxy mechanism ======================')

                event = msg.get("event")  # Expected event field
                data = msg.get("payload")  # Additional application-specific data
                logger.debug(f"Received  msg from fluidity of event { event }: { data }")

                match event:
                    case MessageEvents.MESSAGE_TO_FLUIDITY.value:
                        # Send the message to cluster fluidity
                        logger.debug(f"Sending to fluidity: {self.state.configuration.cluster} > {data}")
