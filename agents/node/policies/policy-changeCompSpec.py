...
async def plan(context, application_description, system_description, mechanisms, telemetry, ml_connector):
    ...
    # TODO: Change this.
    node = 'mls-ubiw-2'
    #logger.info(f'first node found is {node}')
    logger.info(f'Curr change is {curr_change}')

    # NOTE: If we keep the cyclic thing we also need to check if the new value
    # is the same as the current, if yes we need the next.
    # if context['curr_comp_idx'] == 1:
    #     context['curr_comp_idx'] = 0
    # else:
    #     context['curr_comp_idx'] = 1
    context['curr_comp_idx'] = 0

    component = application['spec']['components'][context['curr_comp_idx']]
    comp_name = component['metadata']['name']
    logger.info(f"Will modify component with idx {context['curr_comp_idx']}")
    logger.info(f"Current comp name {comp_name}")
    # logger.info(f'Current comp spec {component}')
