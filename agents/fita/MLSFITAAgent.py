...
class MLSFITAAgent(MLSAgent):

    def __init__(self):
        # Initialize base MLS agent class
        print("In INIT OF FITA AGENT")
        super().__init__()
    ...
    async def run(self):
        ...
        fita_task = asyncio.create_task(self.fita_message_listener())
        self.running_tasks.append(fita_task)
    ...
    async def fita_message_listener(self):
        logger.debug(f"FITA Agent:::: Starting fita message listener.... ")
        while True:
            try:
                msg = await self.mechanisms_controller.queues['fita']['inbound'].get()

                event = msg.get("event")
                data = msg.get("payload")
                logger.debug(f"Received msg from fita event { event }: { data }")

                match event:
                    case _:
                        logger.error(f"Received msg from fita with no event")

            except asyncio.CancelledError:
                logger.debug(f"fita listener: CancelledError")
                break
            except Exception as e:
                logger.error(f"fita: Error processing msg: {e}")
                logger.error(traceback.format_exc())
                await asyncio.sleep(1)
