...
def initialize():
    initialContext = {
        "telemetry": {
            "metrics": ["b1_node1_temperature_0_property_sensor_value"],
...
async def plan(context, application_description, system_description, mechanisms, telemetry, ml_connector):
    current_state = "LOW"
    print(telemetry)
    # FITA command options: LOW,MED, HIGH
    if current_state == "LOW":
        fita_command = {
            "event":"CONTROL_KNOB_EVENT",
            "payload":{"control_knob":"TX_PWR","action":"SET","value":"MED"}
        }
    if current_state == "MED":
        fita_command = {
            "event": "CONTROL_KNOB_EVENT",
            "payload": {"control_knob": "TX_PWR", "action": "SET", "value": "LOW"}
        }

    print(f"Fita policy Sending {fita_command}")
    new_plan = {
        "fita": fita_command
    }
